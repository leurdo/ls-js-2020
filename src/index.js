...
function findAllPSiblings(where) {
    const childNodes = where.children;
    let returnArray = [];
    ...
    const nodesProcess = (nodes) => {
        for (let el of nodes) {
            ...
            if (el.nodeType === 3) {
                ...
                if (el.hasChildNodes()) {
                    nodesProcess(el.childNodes);
                }
                tagsClassesCounting(el);
            }
        }
    };

    nodesProcess(childNodes);
 ...
 */
function observeChildNodes(target, fn) {
    ...
    const callback = (mutationsList, observer) => {
        for (let mutation of mutationsList) {
            ...
            if (mutation.addedNodes.length) {
                fn({
                    type: 'insert',
                    nodes: convertToArray(mutation.addedNodes),
                })
            }
            if (mutation.removedNodes.length) {
                fn({
                    type: 'remove',
                    nodes: convertToArray(mutation.removedNodes),
                })
            }
        }
    };
